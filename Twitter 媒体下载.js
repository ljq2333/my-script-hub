...
/* jshint esversion: 8 */
const filename =
  "twitter_{user-name}(@{user-id})_{date-time}{file-type}";
const TMD = (function () {
  ...
  return {
    ...
    click: async function (btn, status_id, is_exist, index) {
      ...
      let out = ((await GM_getValue("filename", filename) || filename) || "twitter_{user-name}(@{user-id})_{date-time}{file-type}").split("\n").join("");
      console.log('TMD Debug - out value:', out);
      ...
      if (medias.length > 0) {
        console.log('TMD Debug - medias found:', medias.length);
        console.log('TMD Debug - out template:', out);
        let tasks = medias.map((media, i) => {
          ...
          console.log('TMD Debug - filename:', info.out);
        ...
        this.downloader.add(
          ...
          is_exist,
          status_id,
          GM_getValue("enable_packaging", true),
        );
      } else {
        this.status(btn, "failed", "MEDIA_NOT_FOUND");
      }
      ...
      return {
        add: function (
          ...
          if (taskList.length > 1) {
                            ...
                            GM_download({
                              ...
                              onerror: (err) => {
                                ...
                                this.status(
                                  btn,
                                  "failed",
                                  err.details?.current || "ZIP download failed",
                                );
                              },
                            });
                          } else {
            ...
            console.log('TMD Debug - GM_download name:', taskList[0].name);
            tasks.push(taskList[0]);
            thread++;
            this.update();
            fetch(taskList[0].url)
              .then(res => {
                if (!res.ok) throw new Error("HTTP " + res.status);
                return res.blob();
              })
    ...
    settings: async function () {
      ...
      let show_enable_packaging = $element(
        options,
        "label",
        "display: block; margin: 10px;",
        lang.enable_packaging,
      );
      let show_enable_packaging_input = $element(
        show_enable_packaging,
        "input",
        "float: left;",
        "checkbox",
      );
      show_enable_packaging_input.checked = await GM_getValue(
        "enable_packaging",
        true,
      );
...
`,
    ...
    fetchJson: async function (status_id) {
      ...
      let url = encodeURI(
        `${base_url}?variables=${JSON.stringify(variables)}&features=${JSON.stringify(features)}`,
      );
    ...
    language: {
      en: {
        ...
        quote: "Quoted Tweet",
        cancel: "Cancel",
        choose: "Select media to download",
      },
      ja: {
        download: "ダウンロード",
        completed: "ダウンロード完了",
        ...
        enable_packaging: "複数ファイルを ZIP にパッケージ化する",
        original: "元のツイート",
        quote: "引用ツイート",
        cancel: "キャンセル",
        choose: "メディアを選択",
